...
describe('release 集成数据库升级', () => {
  it.each([0, 2, 3, 4, 5])('从版本 %i 升级，保留既有数据并补齐源缓存与历史库', async (version) => {
    ...
    if (version > 0) {
      ...
      if ((version === 3 || version === 5)) await legacy.put('sources', { id: 'source', hash: 'existing' });
      if ((version === 4 || version === 5)) await legacy.put('history', { id: 'history', kind: 'pdf', lastVisitedAt: 42 });
      legacy.close();
    }
    vi.resetModules();
    ...
    try {
      ...
      expect(Array.from(db.objectStoreNames)).toEqual(['bookmarkFolders', 'bookmarks', 'documents', 'history', 'libraryPreferences', 'reading', 'sources', 'tasks', 'translations']);
      ...
      if ((version === 3 || version === 5)) expect(await db.get('sources', 'source')).toMatchObject({ hash: 'existing' });
      if ((version === 4 || version === 5)) expect(await db.get('history', 'history')).toMatchObject({ lastVisitedAt: 42 });
    } finally {
      db.close();
      await deleteDB('web-translate');
    }
