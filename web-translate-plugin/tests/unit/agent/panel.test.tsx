...
describe('论文智能体面板', () => {
  ...
  it('用 Markdown 和公式渲染对话，同时保留页码导航与状态控件', () => {
    const html = renderToStaticMarkup(<AgentPanel open pageCount={3} notice="已使用压缩上下文" messages={[{ role: 'assistant', content: '## Answer\n$x^2$ [p:2]', status: 'streaming' }]} busy error={undefined} onAsk={async () => undefined} onStop={() => undefined} onNavigate={() => undefined} onToggle={() => undefined} />);
